...
// A)
const getPlayersOfTeam = team => playersList.filter(player => player.team === team);

// B)
const getPlayersOfPosition = position => playersList.filter(player => player.position === position);

// C)
const sortPlayersByIdentifier = () => {
   return playersList.sort((playerA, playerB) => playerA.identifier - playerB.identifier);
};

// D)
const checkPlayerExistance = (firstName, lastName) => playersList.find(player => player.first_name === firstName && player.last_name === lastName);

// E)
const insertData = newData => {
   return playersList.map(player => {
      const newDataInsertedPlayer = player;
      for (let key in newData) newDataInsertedPlayer[key] = newData[key];
      return newDataInsertedPlayer;
      // the best practice if we wanna manipulate an argument itself in a function?
   });
};
...
console.log(insertData(sampleData));
